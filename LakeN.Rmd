...
## N cycle in the studied lake

Several key process dominate the N cycle of the studied lake. These include nitrification, denitrification and the fixing of atmospheric nitrogen. These three processes are biologically mediated, and their relative role in the transformation of one specie to another greatly depends on physical conditions including radiation and temperature as well as the chemical properties of the lake, which are largely dependent on its limnological cycle. The consideration of these parameters in a limnological perspective is essential for the consideration of the dominant processes and their ecological impact. These processes are briefly described in the section below:
...
```{r NH4 modelling, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
# segment the data to model by dates
# the dates will be segmented into a list containing the relevant dates for every year
...
for (ii in 1:length(x.depth)){
 ...
 # temp3=datesplitter(temp2,min.date=min.date,max.date=max.date)
  ...
  for (jj in 1:length(temp.names)){
    ...
    a = as.double(coef(lm.Nitrate[[jj]])[1])
    b = as.double(coef(lm.Nitrate[[jj]])[2])
...
R2=expression(R^{2})
d=ggplot(Nitrate.model.df, aes(x=factor(as.integer(Depth)), y=r2, colour=as.factor(Year)))+geom_point()
d=d+labs(title=paste("Nitrate build-up Linear Modeling: Janaury 1 to March 30") ,x = "Depth", y = "r2")
...
d=d+scale_x_continuous(limits=c(0,6E5))
...
```{r NH4 modelling all depths, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
...
for (ii in 1:length(x.depth)){
 ...
 # temp3=yearbyelement(input=temp3)
  ...
  for (jj in 1:length(temp.names)){
    ...
    a = as.double(coef(lm.NH4[[jj]])[1])
    b = as.double(coef(lm.NH4[[jj]])[2])
...
R2=expression(R^{2})
d=ggplot(NH4.model.df, aes(x=factor(as.integer(Depth)), y=r2, colour=as.factor(Year)))+geom_point()
d=d+labs(title=paste("NH4 build-up Linear Modeling: April 30 to December 1") ,x = "Depth", y = "r2")
d=d+scale_x_discrete("Depth", breaks=seq(from=0, to=40, by=5))
...
d=d+scale_x_continuous(limits=c(0,3.5E5))
d=d+scale_y_continuous(limits=c(0,0.5E7))
d
