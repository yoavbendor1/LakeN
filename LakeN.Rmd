...
d
```
# simulate NH4 build-up in depth 30m in order to estimate reaction rate
```{r parameter estimation, echo=FALSE, warning=FALSE, message=FALSE}
...
```
# simulate Nitrate build-up in depth 10m in order to estimate reaction rate
```{r parameter estimation, echo=FALSE, warning=FALSE, message=FALSE}
# segment the data to model by dates
# the dates will be segmented into a list containing the relevant dates for every year
...
lm1.10.Nitrate=lm(temp3.10$'2010'$Nitrate~unlist(datedif.10[[2]]$'2010'))
lm2.10.Nitrate=lm(temp3.10$'2011'$Nitrate~unlist(datedif.10[[2]]$'2011'))
lm3.10.Nitrate=lm(temp3.10$'2012'$Nitrate~unlist(datedif.10[[2]]$'2012'))
...
d=d+labs(title=paste("Nitrate build-up depth:", x.depth[2],"[m]") ,x = "Days from measurment", y = "Nitrate[quant]")
d=d+geom_smooth(method='lm')
d=d+geom_text(x = 50, y = 6E+07, label = eq.1, parse = TRUE, colour="red")
d=d+geom_text(x = 50, y = 5.5E+07, label = eq.2, parse = TRUE, colour="dark green")
d=d+geom_text(x = 50, y = 5.0E+07, label = eq.3, parse = TRUE, colour="blue")
