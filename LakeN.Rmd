...
# define how much of the mixed element remains
mixing.ratio.NH4=1
mixing.ratio.nitrate=1

# crit.depth defines when the lake is considered fully mixed according to oxycline depth
crit.depth=30

# intialize random noise for variables
mean.rand.NH4=3e5
sd.rand.NH4=mean.rand.NH4/10

# the delay between occurrences of lake mixing
# how frequent can the lake be mixed (in days)
mix.delay=15
mix.delay.ind=0
mix.index=data.dates[1]
first.mix=data.dates[1]
first.mix.delay.ind=100
...
for (ii in 2:length(data.dates)){
  ...
  if (oxycline.depth.date$oxycline_int[ii]<crit.depth & if.index!=1){
    ...
    #        if (first.mix.delay.ind>dur2){
    # NH4 consumption
    #      tmp.NH4$NH4[1:max.depth]=tmp.data$NH4[1:max.depth]+parameters.NH4.consumption$a_best[1:max.depth]*(date.dif.for.model.seq[ii-1])
    #        }
  }
