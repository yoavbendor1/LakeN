...
simulate NH4 build-up between April $30^{th}$ and December $1^{st}$ in 30m depth in order to estimate reaction rate
...
Use the estimated parameters to model the lake N cycle
...
```{r intialize modelling, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
...
# initilize variables for calibration
# dur variables are the duration of each process
dur1=30 #duration of NH4 buildup
dur2=30 #duration of NH4 consumption
dur3=30 #duration of Nitrate buildup
dur4=30 #duration of Nitrate consumption

# crit.depth defines when the lake is considered fully mixed
crit.depth=35

# intialize random noise for vaiables
mean.rand.NH4=1e6
sd.rand.NH4=mean.rand.NH4/2

start.row=as.integer(max.depth+1)
end.row=as.integer(2*max.depth)

for (ii in 2:length(unique(data1.split.interp.quant.df$Date))){
  # remove previous remains from loop 
  rm(tmp.NH4, tmp.nitrate)
  # extract the data for the specific date
  
  tmp.data=model.data[(start.row-max.depth):(end.row-max.depth),]
  # initialize empty variables for loop
  tmp.NH4=data.frame(NH4=rep(0,max.depth))
  tmp.nitrate=data.frame(nitrate=rep(0,max.depth))
  
  # find out if the lake is fully mixed or stratified to determine dominant processe
  if (oxycline.depth.date$oxycline_int[ii]<crit.depth){
    # the lake is stratified
    #epilimnion depths
    epi=seq(from=1, to=oxycline.depth.date$oxycline_int[ii], by=1) 
    #Hypolimnion depths
    hypo=seq(to=max.depth, from=oxycline.depth.date$oxycline_int[ii]+1, by=1)  
    
    # calculations for epilimnion
    for (jj in 1:max(epi)){
      NH4.noise=rnorm(1, mean=mean.rand.NH4, sd=sd.rand.NH4) # noise for data
      plus.minus=sign(rnorm(1, mean=0, sd=1)) #randomly select if noise is added or removed
      if (plus.minus==0) plus.minus=1 # eliminate zero for noise sign
      
      tmp.NH4[jj,1]=tmp.data$NH4[jj]+plus.minus*NH4.noise
      if (tmp.NH4[jj,1]<0) tmp.NH4[jj,1]=0 # if the value is smaller than zero, place zero
    }
    
    # calculations for hypolimnion
    for (jj in min(hypo):max(hypo)){
      NH4.noise=rnorm(1, mean=mean.rand.NH4, sd=sd.rand.NH4) # noise for data
      plus.minus=sign(rnorm(1, mean=0, sd=1)) #randomly select if noise is added or removed
      if (plus.minus==0) plus.minus=1 # eliminate zero for noise sign
      
      tmp.NH4$NH4[jj]=tmp.data$NH4[jj]+parameters.NH4.buildup$a_best[jj]*date.dif.for.model.seq[ii-1]+plus.minus*NH4.noise
      
      if (tmp.NH4[jj,1]<0) tmp.NH4[jj,1]=0
    }
  }
  
model.data$NH4[start.row:end.row]=tmp.NH4$NH4
...
```{r temp,eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}

#this is the mixing code (needs to be time-limited)

#  if (oxycline.depth.date$oxycline_int[ii]>crit.depth){
    # the lake is mixed
#    sum.NH4=sum(tmp.data$NH4)
    # redistribute NH4 following a quatratic distribution from 1 at depth 0 to 0 at max.depth
...
#    r = (max.depth^2*(seq(from=1, to=max.depth, by=1)-max.depth)^2)
 #   r=r/sum(r)
  ...
  #  d = abs(r-0.5);
#    idx = which.min(d);
#    tmp.NH4$NH4 = round(sum.NH4*r);
 #   tmp.NH4$NH4[idx] = tmp.NH4$NH4[idx]+(sum.NH4-sum(tmp.NH4$NH4))
...
```{r visualize model results, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
#initilize NH4 plot
