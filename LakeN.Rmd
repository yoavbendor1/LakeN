...
1. Nitrate buildup during lake stratification in the epylimnion
2. Ammonification buildup during lake stratification in the hypolimnion
3. Lake mixing
...
Due to the time consuming nature of this process and the difficulties to modell several coupled processes, only NH4 is modelled here
...
The following plots provide some visual estimation of the data to visualize time-depth processes for further considerations for the modelling of the processes. The depth of the Oxycline is inferred by looking for the steepest part of oxygen concentrations as well as the differences between the epilimnion and the hypolimnion, and is marked by a red line, this will be used to define the dominant process in each of the lake's parts according to its oxygen concentrations
...
# calculate the oxycline depth using findoxycline function
oxycline.depth=findoxylcline(input=data1.split, max.depth=40,oxygen.data.col=9, p.value=0.05, threshold = 5,threshold.depth = 30, depth.interval=15,smooth.param=0.04)

oxycline.depth.date=data.frame(as.Date(unique(data1.split.interp.quant.df$Date)), oxycline.depth[[1]], oxycline.depth[[2]])

colnames(oxycline.depth.date)=c("Date", "oxycline_int", "oxycline_double")
...
image2D(z,x,y, main="Oxygen [concentration] and Oxycline",xlab='',ylab="depth [m]",ylim=rev(range(y)),xaxt="n")
...
The following plots provide som basic iilustration of the dominant processes in the lake. It can be observed that the concentrations of N species is dictated by mixing and stratification. The consumption and development of each specie can be broken into several phases within the time domain. In other words, the processes can be broken down into phases of development, mixing and consumption of the elements.
...
The following plots provide some visual estimation of data correlation and their significance. It can be observed that the dominant effect of oxygen concentration, which dictates the buildup of $NH_4^+$ and $NO_3$. The additional process that dictates their distribution is the physical mixing of the lake, which follows an  emprically fitted quadratic distribution in this model
...
Nitrification and denitrification are two oposing processes that take place in the present or absence of oxygen. They therefore develop in the upper (nitrification) and lower (denitrification) parts of the lake during episodes of a stratified water column. The model therefore relies on the assumprion that their development depends on the presence of oxygen and the time available for their accumulation. Lake mixing, which takes place during winter, disrupts thier buildup, and uniformly distribute them across the lake, and mixing propagates towards the lake bottom. These two proceeses (stratification and mixing)can be examined in detail in two representative depths of 10m (nitrification) and 30m (denitrification). In the following section each of the processes is assumed to follow a linear fit. This assumption is tested via model fitting estimation described in the following section
...
It can be observed from the figures presented above that nitrification is dominant in the upper portions of the lake (depicted for 10m depth), primarily between January and April. From April thtough August nitrate is consumed by primary producers. Ammonium, develops during the stratified lake conditions in its hypolimnion (depicted for depth 30 m), and peaks at during December, right before the mixing of the lake when the deeper lake is oxygenated once more. The ammonium is readily consumed by the producers in the epiliomnion after its mixing. In the following section the time-dependent development of each of the processes is estimated in order to decide whether a linear fit is suited to simulate each of the buildup and consumption processes.

## Simulate nitrate buildup between January $1^{st}$ and March $30^{th}$ in 10m depth in order to estimate processes rate
...
d=d+labs(title=paste("Nitrate buildup depth:", x.depth[1],"[m]", "Janaury 1 to March 30") ,x = "Days from measurment", y = "Nitrate[quant]")
...
## Simulate NH4 buildup between April $30^{th}$ and December $1^{st}$ in 30m depth in order to estimate reaction rate

```{r NH4 buildup modelling, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
# segment the data to model by dates
# the dates will be segmented into a list containing the relevant dates for every year
...
d=d+labs(title=paste("NH4 buildup depth:", x.depth[2],"[m]", "April 30 to December 1") ,x = "Days from measurment", y = "NH4[quant]")
d=d+geom_smooth(method='lm')
...
After consideration of the dominant processes that control nitrate and ammonium content in the lake, the following section provides an assesment of the processes and their fit to a linear model separately for  each depth. The $R^2$ value can be used to asses the best fit of the model for each depth. It can be observed that the quality of the fit varries greatly amongst different years. This elucidates the effect of other processes that are not considered in this project. These include varying nitrate additions from the lake's sorrounding, but also relates to the hydrological differences, since every year the lake receives different amounts of water, and therefore the amount of water that leaves it through the southern dam also varies. Furtheremore, variations in lake level also introduce some compexities since they affect the range of the hypsometric curve that is used for the calculations. These inherent complexities are not fully solved within the scope of this project.


It can be observed, for example, that ammonium buildup does fit the linear model in most depths as nicely as nitrate. Intraannual changes are significatn, and once more illustrate the complexity of the system and its divergence from linearity. Unsurprisingly, the model has the best fit for depths of 20 to 35m, where oxygenation solely depends on annual stratification and mixing.

## Nitrate buildup during stratification between January $1^st$ and March $30^th$
...
Nitrate.model.df=data.frame(matrix(data=NA, ncol=5, nrow=40*length(temp.names)))
...
```{r Nitrate buildup parameter presentation, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
R2=expression(R^{2})

d=ggplot(Nitrate.model.df, aes(x=factor(as.integer(Depth)), y=r2, colour=as.factor(Year), shape=as.factor(Year)))+geom_point()
d=d+labs(title=paste("Nitrate buildup Linear Modeling: Janaury 1 to March 30") ,x = "Depth [m]", y = "r2")
...
d=d+labs(title=paste("Nitrate buildup Linear Modeling: Janaury 1 to March 30") ,x = "b", y = "a")
...
d=d+labs(title=paste("Nitrate buildup Linear Modeling: Janaury 1 to March 30") ,x = "Depth [m]", y = "a")
d=d+scale_colour_gradient2(name=R2, low = "blue", mid = "green" , high = "red", midpoint=0.8)
d=d+scale_shape_discrete(name="Year")
d
...
d=d+labs(title=paste("a parameter for Nitrate buildup: Janaury 1 to March 30") ,x = "Year", y = "a")
...
d=d+labs(title=paste("Nitrate buildup Linear Modeling: Janaury 1 to March 30") ,x = "Depth [m]", y = "b")
d=d+scale_colour_gradient2(name=R2, low = "blue", mid = "green" , high = "red", midpoint=0.8)
d=d+scale_shape_discrete(name="Year")
d

d=ggplot(Nitrate.model.df, aes(x=as.factor(Year), y=as.double(b), colour=as.factor(Year)))+geom_boxplot()
d=d+labs(title=paste("b parameter for Nitrate buildup: Janaury 1 to March 30") ,x = "Year", y = "b")
d=d+scale_colour_discrete(name="Year")
#d
...
## Ammonium buildup during stratification between April $30^th$ and December $12^th$


```{r NH4 buildup modelling all depths, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
...
temp.df=data.frame(matrix(data=NA, ncol=5, nrow=1))
colnames(temp.df) = colnames((model.param[[temp.names[1]]]))
NH4.model.df=data.frame(matrix(data=NA, ncol=5, nrow=40*length(temp.names)))
...
```{r NH4 buildup parameter presentation, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
R2=expression(R^{2})
d=ggplot(NH4.model.df, aes(x=factor(as.integer(Depth)), y=r2, colour=as.factor(Year), shape=as.factor(Year)))+geom_point()
d=d+labs(title=paste("NH4 buildup Linear Modeling: April 30 to December 1") ,x = "Depth [m]", y = "r2")
d=d+scale_x_discrete("Depth", breaks=seq(from=0, to=40, by=5))
...
d=d+labs(title=paste("NH4 buildup Linear Modeling: April 30 to December 1") ,x = "b", y = "a")
...
d=d+labs(title=paste("NH4 buildup Linear Modeling: April 30 to December 1") ,x = "Depth [m]", y = "a")
d=d+scale_colour_gradient2(name=R2, low = "blue", mid = "green" , high = "red", midpoint=0.5)
d=d+scale_shape_discrete(name="Year")
d
...
d=d+labs(title=paste("a parameter for NH4 buildup: April 30 to December 1") ,x = "Year", y = "a")
d=d+scale_colour_discrete(name="Year")
d


d=ggplot(NH4.model.df, aes(x=Depth, y=as.double(b), colour=r2, shape=as.factor(Year)))+geom_point()
d=d+labs(title=paste("NH4 buildup Linear Modeling: April 30 to December 1") ,x = "Depth [m]", y = "b")
d=d+scale_colour_gradient2(name=R2, low = "blue", mid = "green" , high = "red", midpoint=0.5)
d=d+scale_shape_discrete(name="Year")
d
...
d=d+labs(title=paste("b parameter for NH4 buildup: April 30 to December 1") ,x = "Year", y = "b")
d=d+scale_colour_discrete(name="Year")
d
...
## Ammonium consumption after mixing between January $15^{th}$ to March $15^{th}$
...
#define min and max date for modelling
min.date=as.Date("2010-01-15")
max.date=as.Date("2010-03-15")

# initialize empty lists for model parameters collection
lm.NH4.b=vector("list", length=0)
model.param=vector("list", length=0)
NH4.model.b=vector("list", length=0)
...
for (ii in 1:length(x.depth)){
  ...
  temp.names=names(temp3)
  for (jj in 1:length(temp.names)){
    lm.NH4.b[[temp.names[jj]]]=lm(temp3[[jj]]$NH4~unlist(datedif[[2]][[jj]]))
    
    a = as.double(coef(lm.NH4.b[[jj]])[2])
    b = as.double(coef(lm.NH4.b[[jj]])[1])
    r2 = as.double(summary(lm.NH4.b[[jj]])$r.squared)
  ...
  NH4.model.b[[ii]]=model.param
}

temp.df=data.frame(matrix(data=NA, ncol=5, nrow=1))
colnames(temp.df) = colnames((model.param[[temp.names[1]]]))
NH4.model.b.df=data.frame(matrix(data=NA, ncol=5, nrow=40*length(temp.names)))
colnames(NH4.model.b.df) = colnames((model.param[[temp.names[1]]]))
ll=seq(from=1, to=nrow(NH4.model.b.df), by=length(NH4.model.b[[1]]))

for (ii in 1:length(NH4.model.b)){
  for (jj in 1:length(NH4.model.b[[ii]])){
    temp.df=NH4.model.b[[ii]][[jj]]
    NH4.model.b.df[ll[ii]+jj-1,1:5]=temp.df
  }
}

NH4.model.b.df$Year=as.double(NH4.model.b.df$Year)
summary(NH4.model.b.df[,1:4])
...
d=ggplot(NH4.model.b.df, aes(x=factor(as.integer(Depth)), y=r2, colour=as.factor(Year), shape=as.factor(Year)))+geom_point()
...
d=ggplot(NH4.model.b.df, aes(x=as.double(b), y=as.double(a), colour=r2, shape=as.factor(Year)))+geom_point()
...
d=ggplot(NH4.model.b.df, aes(x=Depth, y=as.double(a), colour=r2, shape=as.factor(Year)))+geom_point()
...
d=ggplot(NH4.model.b.df, aes(x=as.factor(Year), y=as.double(a), colour=as.factor(Year)))+geom_boxplot()
d=d+labs(title=paste("a NH4 consumption: January 10 to March 15") ,x = "Year", y = "a")
d=d+scale_colour_discrete(name="Year")
d

d=ggplot(NH4.model.b.df, aes(x=Depth, y=as.double(b), colour=r2, shape=as.factor(Year)))+geom_point()
d=d+labs(title=paste("NH4 consumption Linear Modeling: January 10 to March 15") ,x = "Depth [m]", y = "b")
d=d+scale_colour_gradient2(name=R2, low = "blue", mid = "green" , high = "red", midpoint=0.5)
d=d+scale_shape_discrete(name="Year")
d

d=ggplot(NH4.model.b.df, aes(x=as.factor(Year), y=as.double(b), colour=as.factor(Year)))+geom_boxplot()
...
```{r nitrate consumption all depths, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
...
model.param=vector("list", length=0)
nitrate.model.b=vector("list", length=0)
...
for (ii in 1:length(x.depth)){
  ...
  nitrate.model.b[[ii]]=model.param
}

temp.df=data.frame(matrix(data=NA, ncol=5, nrow=1))
colnames(temp.df) = colnames((model.param[[temp.names[1]]]))
nitrate.model.b.df=data.frame(matrix(data=NA, ncol=5, nrow=40*length(temp.names)))
colnames(nitrate.model.b.df) = colnames((model.param[[temp.names[1]]]))
ll=seq(from=1, to=nrow(nitrate.model.b.df), by=length(nitrate.model.b[[1]]))

for (ii in 1:length(nitrate.model.b)){
  for (jj in 1:length(nitrate.model.b[[ii]])){
    temp.df=nitrate.model.b[[ii]][[jj]]
    nitrate.model.b.df[ll[ii]+jj-1,1:5]=temp.df
  }
}

nitrate.model.b.df$Year=as.double(nitrate.model.b.df$Year)
summary(nitrate.model.b.df[,1:4])
...
d=ggplot(nitrate.model.b.df, aes(x=factor(as.integer(Depth)), y=r2, colour=as.factor(Year), shape=as.factor(Year)))+geom_point()
...
d=ggplot(nitrate.model.b.df, aes(x=as.double(b), y=as.double(a), colour=r2, shape=as.factor(Year)))+geom_point()
...
d=ggplot(nitrate.model.b.df, aes(x=Depth, y=as.double(a), colour=r2, shape=as.factor(Year)))+geom_point()
...
d=ggplot(nitrate.model.b.df, aes(x=as.factor(Year), y=as.double(a), colour=as.factor(Year)))+geom_boxplot()
d=d+labs(title=paste("a Nitrate consumption: April 15 to July 1") ,x = "Year", y = "a")
d=d+scale_colour_discrete(name="Year")
d

d=ggplot(nitrate.model.b.df, aes(x=Depth, y=as.double(b), colour=r2, shape=as.factor(Year)))+geom_point()
d=d+labs(title=paste("Nitrate consumption Linear Modeling: April 15 to July 1") ,x = "Depth [m]", y = "b")
d=d+scale_colour_gradient2(name=R2, low = "blue", mid = "green" , high = "red", midpoint=0.5)
d=d+scale_shape_discrete(name="Year")
d

d=ggplot(nitrate.model.b.df, aes(x=as.factor(Year), y=as.double(b), colour=as.factor(Year)))+geom_boxplot()
...
```{r collect all parameters, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}

template=data.frame(cbind(Depth=seq(from=1, to=40, by=1), a_best=rep(NA, 40), a_min=rep(NA, 40), a_max=rep(NA, 40), b_best=rep(NA, 40), b_min=rep(NA, 40), b_max=rep(NA, 40)))

parameters.nitrate.buildup=template
parameters.NH4.buildup=template
parameters.nitrate.consumption=template
parameters.NH4.consumption=template

temp=vector("list", length=0)
temp=split(Nitrate.model.df,f=Nitrate.model.df$Depth)

for (ii in 1:length(temp)){
  parameters.nitrate.buildup$a_best[ii]=temp[[ii]]$a[which.max(temp[[ii]]$r2)]
  parameters.nitrate.buildup$a_max[ii]=max(temp[[ii]]$a)
  parameters.nitrate.buildup$a_min[ii]=min(temp[[ii]]$a)
  
  parameters.nitrate.buildup$b_best[ii]=temp[[ii]]$b[which.max(temp[[ii]]$r2)]
  parameters.nitrate.buildup$b_max[ii]=max(temp[[ii]]$b)
  parameters.nitrate.buildup$b_min[ii]=min(temp[[ii]]$b)
}

temp=vector("list", length=0)
temp=split(NH4.model.df,f=NH4.model.df$Depth)

for (ii in 1:length(temp)){
  parameters.NH4.buildup$a_best[ii]=temp[[ii]]$a[which.max(temp[[ii]]$r2)]
  parameters.NH4.buildup$a_max[ii]=max(temp[[ii]]$a)
  parameters.NH4.buildup$a_min[ii]=min(temp[[ii]]$a)
  
  parameters.NH4.buildup$b_best[ii]=temp[[ii]]$b[which.max(temp[[ii]]$r2)]
  parameters.NH4.buildup$b_max[ii]=max(temp[[ii]]$b)
  parameters.NH4.buildup$b_min[ii]=min(temp[[ii]]$b)
}

temp=vector("list", length=0)
temp=split(nitrate.model.b.df,f=nitrate.model.b.df$Depth)

for (ii in 1:length(temp)){
  parameters.nitrate.consumption$a_best[ii]=temp[[ii]]$a[which.max(temp[[ii]]$r2)]
  parameters.nitrate.consumption$a_max[ii]=max(temp[[ii]]$a)
  parameters.nitrate.consumption$a_min[ii]=min(temp[[ii]]$a)
  
  parameters.nitrate.consumption$b_best[ii]=temp[[ii]]$b[which.max(temp[[ii]]$r2)]
  parameters.nitrate.consumption$b_max[ii]=max(temp[[ii]]$b)
  parameters.nitrate.consumption$b_min[ii]=min(temp[[ii]]$b)
}

temp=vector("list", length=0)
temp=split(NH4.model.b.df,f=NH4.model.b.df$Depth)


for (ii in 1:length(temp)){
  parameters.NH4.consumption$a_best[ii]=temp[[ii]]$a[which.max(temp[[ii]]$r2)]
  parameters.NH4.consumption$a_max[ii]=max(temp[[ii]]$a)
  parameters.NH4.consumption$a_min[ii]=min(temp[[ii]]$a)
  
  parameters.NH4.consumption$b_best[ii]=temp[[ii]]$b[which.max(temp[[ii]]$r2)]
  parameters.NH4.consumption$b_max[ii]=max(temp[[ii]]$b)
  parameters.NH4.consumption$b_min[ii]=min(temp[[ii]]$b)
}
```

# Lake Modelling

In the following section the previously estimated parameters are used to simulate annual $NH_4^+$ development in the lake. The parameters that are used for this model include the linear fit parameters that were calculated in the previous section, as well as several parameters that define the intensity of the lake mixing, and the durations of the delay in the consumption of $NH_4^+$, and the consumption itself.

```{r define modelling parameters, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}
...
#extract only relevant data for the model
data.for.model=data1.split.interp.quant.df[,c("Date", "Depth", "NH4", "Nitrate")]

#initialize data frame for model
model.data=data.frame(matrix(data=NA, ncol=ncol(data.for.model), nrow=nrow((data.for.model))))
colnames(model.data)=colnames(data.for.model)
model.data$Date=data.for.model$Date
model.data$Depth=data.for.model$Depth
rownames(data.for.model)=seq(from=1, to=nrow(data.for.model), by=1)
rownames(model.data)=seq(from=1, to=nrow(model.data), by=1)

#initialize constants and first data points
max.depth=40
model.data[1:40,]=data.for.model[1:40,]

# create date-difference vector in days for modelling  
tmp1=data.frame(Date=rep(NA, length(unique(data1$Date))))
tmp1$Date=as.Date(unique(data1$Date))
tmp1=yearassigner(tmp1)

date.dif.for.model=datedifferencedf(tmp1)
date.dif.for.model.seq = unlist(date.dif.for.model[[1]])
data.dates=as.Date(unique(data.for.model$Date))

parameters.NH4.consumption.2=parameters.NH4.consumption$a_best
parameters.NH4.buildup.2=parameters.NH4.buildup$a_best

model.data.2=lakeNH4(input=model.data,max.depth=40, mean.rand.NH4=3e5, sd.rand.NH4=3e5/5, mix.index=as.Date("2010-06-06"), data.dates=as.Date(unique(data.for.model$Date)), mix.delay.ind=0, first.mix=as.Date("2011-06-05"), NH4.comsumption.correction.factor=3, parameters.NH4.consumption=parameters.NH4.consumption.2, date.dif.for.model.seq=date.dif.for.model.seq, oxycline.depth.date=oxycline.depth.date,mixing.ratio.NH4=0.9,crit.depth=35, parameters.NH4.buildup=parameters.NH4.buildup.2, dur2=30, dur3=120, first.mix.delay.ind=1.5*dur3)

X=model.data[c('Depth','NH4')]
test=apply(X, MARGIN=1, FUN='lakeNH4',max.depth=40,input=X,mean.rand.NH4=seq(from=3e5, to=3e7, by=1000), sd.rand.NH4=seq(from=3e4, to=3e6, by=1000), mix.index=as.Date("2010-06-06"), data.dates=as.Date(unique(data.for.model$Date)), mix.delay.ind=0, first.mix=as.Date("2011-06-05"), NH4.comsumption.correction.factor=seq(from=0.5, to=10, by=0.5), parameters.NH4.consumption=parameters.NH4.consumption.2, date.dif.for.model.seq=date.dif.for.model.seq, oxycline.depth.date=oxycline.depth.date,crit.depth=seq(from=20, to=40, by=1), parameters.NH4.buildup=parameters.NH4.buildup.2, dur2=seq(from=20 ,to=60, by=5), dur3=seq(from=60 ,to=150, by=5), first.mix.delay.ind=1.5*dur3)

NSE.mat=as.data.frame(rep(0, length(test)))
for (ii in 1:length(test)){
  tmp=test[[ii]]$NH4
  tmp[is.na(tmp)]=0
NSE.mat[ii,1]=NSE(m=tmp,o=data.for.model$NH4)
}

length(which(NSE.mat>0))
#NH4 plot
x.depth=seq(from=5, to=40, by=5)
depth.indices=which(match(model.data.2$Depth,x.depth[1])==1)
pnh4=ggplot(data=model.data.2[depth.indices,], aes(x=Date, y=NH4, colour=Depth))+geom_line()
pnh4=pnh4+labs(title="modelled NH4" ,x = "Month", y = "modelled NH4[quant]")
pnh4=pnh4+scale_x_date(date_labels="%m-%Y", date_breaks="6 months")
pnh4=pnh4+scale_colour_gradient2(low = "red", mid = "green" , high = "blue", midpoint = 20)
for (ii in 2:length(x.depth)){
  depth.indices=which(match(model.data.2$Depth,x.depth[ii])==1)
  pnh4=pnh4+geom_line(data=model.data.2[depth.indices,], aes(x=Date, y=NH4, colour=Depth))
}
pnh4

depth.indices=which(match(data.for.model$Depth,x.depth[1])==1)
pnh4=ggplot(data=model.data[depth.indices,], aes(x=Date, y=NH4, colour=Depth))+geom_line()
pnh4=pnh4+labs(title="NH4" ,x = "Month", y = "NH4[quant]")
pnh4=pnh4+scale_x_date(date_labels="%m-%Y", date_breaks="6 months")
pnh4=pnh4+scale_colour_gradient2(low = "red", mid = "green" , high = "blue", midpoint = 20)
for (ii in 2:length(x.depth)){
  depth.indices=which(match(data.for.model$Depth,x.depth[ii])==1)
  pnh4=pnh4+geom_line(data=data.for.model[depth.indices,], aes(x=Date, y=NH4, colour=Depth))
}
pnh4

# plot NH4 values
# create x (date) and y (depth) vectors for plots
x= as.numeric(model.data.2$Date)
dim(x)=c(40,length(x)/40)
y= rep(seq(from=1, to=40, by=1), length(x)/40)
dim(y)=c(40,length(y)/40)
...
z= model.data.2$NH4
dim(z)=c(40,length(z)/40)
image2D(z,x,y, main="modelled NH4 [amount]",xlab='',ylab="depth [m]",ylim=rev(range(y)),xaxt="n")
r <- as.POSIXct(range(model.data.2$Date), "weeks")
axis.Date(1, at = seq(r[1], r[2], by = "month"), format = "%m-%Y")
lines(unique(x),oxycline.depth[[2]] ,col="red",lwd=2)
...
# plot NH4 values
# create x (date) and y (depth) vectors for plots
z= data.for.model$NH4
dim(z)=c(40,length(z)/40)
image2D(z,x,y, main="NH4 [amount]",xlab='',ylab="depth [m]",ylim=rev(range(y)),xaxt="n")
r <- as.POSIXct(range(data.for.model$Date), "weeks")
axis.Date(1, at = seq(r[1], r[2], by = "month"), format = "%m-%Y")
lines(unique(x),oxycline.depth[[2]] ,col="red",lwd=2)
...
```{r define modelling parameters2, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, out.width = '600px', out.height = '300px'}

#start a loop for the modelling
for (ii in 2:length(data.dates)){
  ...
  if (first.mix.delay.ind>dur2 & first.mix.delay.ind<dur3){
    indicator[ii,]=4
    tmp.NH4$NH4[1:max.depth]=tmp.data$NH4[1:max.depth]+NH4.comsumption.correction.factor*parameters.NH4.consumption$a_best[1:max.depth]*(date.dif.for.model.seq[ii-1])
   if.index=1
    num.mix=num.mix+1
  }
  ...
  if (oxycline.depth.date$oxycline_int[ii]<crit.depth & if.index!=1){
    ...
    tmp.NH4$NH4[(epi.depth+1):max.depth]=tmp.data$NH4[(epi.depth+1):max.depth]+parameters.NH4.buildup$a_best[(epi.depth+1):max.depth]*(date.dif.for.model.seq[ii-1])
  }
...
#NH4 plot
x.depth=seq(from=5, to=40, by=5)
depth.indices=which(match(model.data$Depth,x.depth[1])==1)
pnh4=ggplot(data=model.data[depth.indices,], aes(x=Date, y=NH4, colour=Depth))+geom_line()
pnh4=pnh4+labs(title="modelled NH4" ,x = "Month", y = "modelled NH4[quant]")
pnh4=pnh4+scale_x_date(date_labels="%m-%Y", date_breaks="6 months")
pnh4=pnh4+scale_colour_gradient2(low = "red", mid = "green" , high = "blue", midpoint = 20)
for (ii in 2:length(x.depth)){
  depth.indices=which(match(model.data$Depth,x.depth[ii])==1)
  pnh4=pnh4+geom_line(data=model.data[depth.indices,], aes(x=Date, y=NH4, colour=Depth))
}
pnh4

# plot NH4 values
# create x (date) and y (depth) vectors for plots
x= as.numeric(model.data$Date)
dim(x)=c(40,length(x)/40)
y= rep(seq(from=1, to=40, by=1), length(x)/40)
dim(y)=c(40,length(y)/40)
...
dim(z)=c(40,length(z)/40)
image2D(z,x,y, main="NH4 [amount]",xlab='',ylab="depth [m]",ylim=rev(range(y)),xaxt="n")
r <- as.POSIXct(range(model.data$Date), "weeks")
axis.Date(1, at = seq(r[1], r[2], by = "month"), format = "%m-%Y")
lines(unique(x),oxycline.depth[[2]] ,col="red",lwd=2)
...
# plot NH4 values
# create x (date) and y (depth) vectors for plots
x= as.numeric(data.for.model$Date)
dim(x)=c(40,length(x)/40)
y= rep(seq(from=1, to=40, by=1), length(x)/40)
dim(y)=c(40,length(y)/40)
